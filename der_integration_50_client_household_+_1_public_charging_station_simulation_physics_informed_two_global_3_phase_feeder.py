...
class FedClient:
    def __init__(self, env):
        self.env    = env
        self.agent  = SAC("MlpPolicy", env, verbose=0, device=device)
        self.obs    = env.reset()
        self.prev_V = 0.0
        self.ev_r   = 0.0
...
def fed_loop(
    ...
    for r in range(n_rounds):
        ...
        new_ws = []
        for c in clients:
            c.agent.learn(total_timesteps=local_steps, log_interval=None)
            new_ws.append(c.agent.get_parameters())
        pub_agent.learn(total_timesteps=local_steps, log_interval=None)
